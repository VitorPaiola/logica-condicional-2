...
public class MaiorMenorNumeroV4 {
    ...
    public static void exibirMaiorMenorNumero(List<Integer> numeros) {
        if (numeros.isEmpty()) {
            ...
            int[] maiorMenor = { Integer.MIN_VALUE, Integer.MAX_VALUE };
            numeros.stream().forEach(numero -> {
                if (numero > maiorMenor[0]) maiorMenor[0] = numero;
                if (numero < maiorMenor[1]) maiorMenor[1] = numero;
            });

            System.out.println("Maior numero: " + maiorMenor[0]);
            System.out.println("Menor numero: " + maiorMenor[1]);
        }
    }
}
